...
tags: [classloader, execution, runtimedataarea, constantpool] # TAG names should always be lowercase
---

## ✅ JVM 원리

JVM을 실행하면 RAM의 heap위에 올라가게 된다.<br>
...
1. Programmer codes java `source code(.java)`
2. Compiler compiles `source code(.java)` into `java byte code(.class)`
   the code is consisted of Opcode
3. Transmit compiled `bite code` to **class loader**
4. Class loader uses **Dynamic Loading** to load and link needed classes
   and puts it on **run time data area**, which is **JVM memory**
5.
...
⭐️ CLASS LOADER단계에서 `static 변수`와 `함수`가 실행된다
...
## 💡 JVM 구성 요소: 🛠️ Execution Engine 수행
...
## 💡 JVM 구성 요소: 📦 JVM Runtime Data Area 저장
...
➖ 상수(final static)<br>  
➖ 특히, 상수(static)는 runtime 때 만들어지는 것이 아니라❌ 컴파일 때 만들어진다. ⭕️<br>
➖ 문자열 Literal 여기에 저장<br>

#### ✔️ HEAP

런타임 시, 동적으로 할당되는 영역으로 **인스턴스와 배열 타입 등의 주소 저장**<br>
➖ 인스턴스 여기에 저장<br>
➖ 인스턴스 변수(instance variable) 저장<br>
<br>
